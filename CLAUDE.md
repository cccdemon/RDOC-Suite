...
6. **`PUBLIC_BASE_PATH` macht die Bridge reverse-proxy-aware** ([apps/bridge/src/config/env.ts:17-23](apps/bridge/src/config/env.ts#L17-L23)). Produktion auf `suite.raumdock.org` laeuft am Host-Root, daher bleibt `PUBLIC_BASE_PATH=""` und OAuth-Cookies liegen unter `/auth`. Nur falls spaeter ein Path-Prefix mit Strip-Proxy verwendet wird, muss `PUBLIC_BASE_PATH` auf diesen Prefix gesetzt werden. Zod-Schema akzeptiert nur `""` oder Werte mit fuehrendem `/`.
