...
export function LateArrival({
  ...
  const marker = eta ? (
    <span
      ...
      title="Kommt später"
      style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "1px 7px", fontFamily: MONO, fontSize: "0.6rem", letterSpacing: "0.04em", borderRadius: 4, border: "1px solid rgba(240,165,0,0.5)", background: "rgba(240,165,0,0.12)", color: "#f0a500", whiteSpace: "nowrap" }}
    >
      ...
      {open ? (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }} onClick={(e) => e.stopPropagation()}>
          <input
            ...
            style={{ background: "#0e1926", border: "1px solid rgba(240,165,0,0.4)", color: "#ccdde8", fontFamily: MONO, fontSize: "0.66rem", padding: "0.12rem 0.3rem", borderRadius: 5, outline: "none" }}
          />
          <button type="button" data-testid={testid ? `${testid}-save` : undefined} title="Speichern" onClick={() => { if (/^([01]\d|2[0-3]):[0-5]\d$/.test(val)) { onSet(val); setOpen(false); } }} style={{ display: "inline-flex", padding: "0.15rem 0.3rem", border: "1px solid rgba(0,255,136,0.4)", background: "rgba(0,255,136,0.1)", color: "#00ff88", borderRadius: 5, cursor: "pointer" }}><Ic name="check" size={11} sw={2} /></button>
          <button type="button" title="Abbrechen" onClick={() => { setOpen(false); setVal(eta ?? ""); }} style={{ display: "inline-flex", padding: "0.15rem 0.3rem", border: "1px solid rgba(255,255,255,0.12)", background: "transparent", color: "#7e92a4", borderRadius: 5, cursor: "pointer" }}><Ic name="x" size={11} sw={2} /></button>
        </span>
      ) : (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }} onClick={(e) => e.stopPropagation()}>
