        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem", marginBottom: canManage ? "0.9rem" : 0 }}>
          {docs.map((d) => (
            <div key={d.id} style={{ display: "flex", alignItems: "center", gap: "0.6rem", padding: "0.45rem 0.6rem", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 8 }}>
              ...
              {canManage && (
                <button type="button" data-testid={`doc-del-${d.id}`} title="Dokument entfernen" disabled={busy} onClick={() => remove(d.id)} style={{ flexShrink: 0, width: 24, height: 24, borderRadius: 6, border: "1px solid rgba(228, 115, 106,0.4)", background: "rgba(228, 115, 106,0.07)", color: "var(--red)", display: "inline-flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}><Ic name="x" size={12} sw={2} /></button>
              )}
            </div>
          ))}
        </div>
