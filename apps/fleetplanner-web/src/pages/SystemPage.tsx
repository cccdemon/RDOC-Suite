...
const STATUS_COLOR: Record<string, string> = { ok: "#00ff88", warn: "#f0a500", error: "#ff4444", down: "#ff4444" };
const STATUS_TEXT: Record<string, string> = { ok: "OK", warn: "Warnung", error: "Fehler", down: "Offline" };
const LEVEL_COLOR: Record<string, string> = { info: "#5fa8d8", warn: "#f0a500", error: "#ff4444" };
...
export function SystemPage({ session }: { session: SessionResponse | null }) {
  ...
  const card: React.CSSProperties = { border: "1px solid rgba(0,212,255,0.16)", borderRadius: 13, background: "#090f18", padding: "1.05rem 1.15rem" };
  const tile = (key: string, name: string, status: string, detail: string, icon: string) => {
    const c = STATUS_COLOR[status] ?? "#9fb1c2";
    return (
      <div key={key} data-testid={`svc-${key}`} style={{ border: "1px solid rgba(0,212,255,0.14)", borderLeft: `3px solid ${c}`, borderRadius: 12, background: "#090f18", padding: "0.85rem 0.95rem", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.5rem" }}>
          <span style={{ display: "flex", alignItems: "center", gap: "0.4rem", fontFamily: MONO, fontSize: "0.62rem", letterSpacing: "0.06em", color: "#c2d2de" }}>
        ...
        <div style={{ fontSize: "0.78rem", color: "#9fb1c2", lineHeight: 1.3 }}>{detail}</div>
      </div>
    );
  };
  ...
  return (
    <div data-testid="system-page" style={{ width: "100%", color: "#ccdde8" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-end", justifyContent: "space-between", gap: "1rem 1.4rem", marginBottom: "1.4rem", paddingBottom: "1.1rem", borderBottom: "1px solid rgba(0,212,255,0.14)" }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.35rem" }}><span style={{ color: "#00d4ff", display: "inline-flex" }}><Ic name="shield" size={19} sw={1.7} /></span><span style={{ fontFamily: MONO, fontSize: "0.66rem", letterSpacing: "0.14em", color: "#5b6b7a" }}>ADMIN // SYSTEM &amp; LOGS</span></div>
          <h1 style={{ fontWeight: 700, fontSize: "1.95rem", lineHeight: 1.05, color: "#eaf4fb", margin: 0 }}>System &amp; Logs</h1>
          <div style={{ color: "#9fb1c2", fontSize: "0.9rem", marginTop: "0.2rem" }}>
            {checkedAt ? <>Stand <span style={{ fontFamily: MONO, color: "#7e92a4" }}>{fmtTs(checkedAt)}</span></> : "—"} · Log-Historie {retentionDays} Tage
          </div>
        </div>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "0.7rem" }}>
          <button type="button" data-testid="live-toggle" onClick={() => setLive((v) => !v)} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "0.5rem 0.85rem", fontFamily: MONO, fontSize: "0.72rem", letterSpacing: "0.03em", borderRadius: 9, cursor: "pointer", whiteSpace: "nowrap", border: `1px solid ${live ? "#00ff8866" : "#5b6b7a66"}`, background: live ? "#00ff8812" : "#0e1926", color: live ? "#00ff88" : "#7e92a4" }}>
          ...
          <button type="button" data-testid="manual-refresh" onClick={() => void refresh()} style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "0.5rem 0.8rem", fontFamily: MONO, fontSize: "0.72rem", borderRadius: 9, cursor: "pointer", border: "1px solid rgba(0,212,255,0.3)", background: "rgba(0,212,255,0.06)", color: "#9fb1c2" }}>
      ...
      {err && (
        <div style={{ ...card, borderColor: "rgba(255,68,68,0.4)", color: "#ff7a7a", marginBottom: "1.1rem", display: "flex", alignItems: "center", gap: "0.6rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: "0.9rem", marginBottom: "1.3rem" }}>
        {syncs.map((s) => {
          const c = STATUS_COLOR[s.status] ?? "#9fb1c2";
          ...
          return (
            <section key={s.key} data-testid={`sync-${s.key}`} style={card}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.55rem", marginBottom: "0.7rem" }}>
                ...
                <span style={{ fontFamily: MONO, fontSize: "0.74rem", letterSpacing: "0.06em", color: "#eaf4fb" }}>{s.name}</span>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.45rem 0.8rem", marginBottom: "0.8rem", fontSize: "0.8rem" }}>
                <div><div style={{ fontFamily: MONO, fontSize: "0.56rem", color: "#5b6b7a" }}>GECACHED</div><div style={{ color: "#c2d2de" }}>{s.count.toLocaleString("de-DE")}</div></div>
                <div><div style={{ fontFamily: MONO, fontSize: "0.56rem", color: "#5b6b7a" }}>LETZTER LAUF</div><div style={{ color: "#c2d2de" }}>{fmtRel(s.lastRun)}</div></div>
                <div style={{ gridColumn: "1 / 3" }}><div style={{ fontFamily: MONO, fontSize: "0.56rem", color: "#5b6b7a" }}>ERGEBNIS</div><div style={{ color: s.lastResult?.includes("ERROR") ? "#ff7a7a" : "#9fb1c2", fontFamily: MONO, fontSize: "0.72rem", wordBreak: "break-word" }}>{s.lastResult ?? "—"}</div></div>
              </div>
      ...
      <section style={{ ...card, padding: 0, overflow: "hidden" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "0.6rem", padding: "0.85rem 1.1rem", borderBottom: "1px solid rgba(0,212,255,0.14)" }}>
          <span style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}><span style={{ color: "#00d4ff", display: "inline-flex" }}><Ic name="doc" size={16} sw={1.6} /></span><span style={{ fontFamily: MONO, fontSize: "0.74rem", letterSpacing: "0.08em", color: "#eaf4fb" }}>EREIGNIS-LOG</span><span style={{ fontFamily: MONO, fontSize: "0.62rem", color: "#5b6b7a" }}>{filteredEvents.length}</span></span>
          <div style={{ marginLeft: "auto", display: "flex", gap: "0.45rem" }}>
            <select data-testid="level-filter" value={levelFilter} onChange={(e) => setLevelFilter(e.target.value)} style={{ background: "#0e1926", border: "1px solid rgba(0,212,255,0.14)", color: "#ccdde8", fontFamily: MONO, fontSize: "0.7rem", padding: "0.3rem 0.45rem", borderRadius: 6, cursor: "pointer" }}>
              <option value="">Alle Level</option>
              <option value="error">Fehler</option>
              <option value="warn">Warnung</option>
              <option value="info">Info</option>
            </select>
            <select data-testid="cat-filter" value={catFilter} onChange={(e) => setCatFilter(e.target.value)} style={{ background: "#0e1926", border: "1px solid rgba(0,212,255,0.14)", color: "#ccdde8", fontFamily: MONO, fontSize: "0.7rem", padding: "0.3rem 0.45rem", borderRadius: 6, cursor: "pointer" }}>
        ...
        {filteredEvents.length === 0 ? (
          <div style={{ padding: "1.6rem 1.1rem", textAlign: "center", color: "#5b6b7a", fontFamily: MONO, fontSize: "0.78rem" }}>Keine Ereignisse im gewählten Zeitraum.</div>
        ) : (
          <div style={{ maxHeight: "60vh", overflowY: "auto" }}>
            {filteredEvents.map((ev) => {
              const lc = LEVEL_COLOR[ev.level] ?? "#9fb1c2";
              ...
              return (
                <div key={ev.id} data-testid={`event-${ev.id}`} style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                  ...
                  >
                    ...
                    <span style={{ fontFamily: MONO, fontSize: "0.66rem", color: "#7e92a4", whiteSpace: "nowrap" }}>{fmtTs(ev.ts)}</span>
                    ...
                    <span style={{ color: ev.level === "error" ? "#ff9a9a" : "#c2d2de", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: open ? "normal" : "nowrap" }}>{ev.message}{ev.detail && !open ? <span style={{ color: "#5b6b7a" }}> …</span> : null}</span>
                  </div>
                  {open && ev.detail && (
                    <pre style={{ margin: 0, padding: "0.4rem 1.1rem 0.8rem 2.6rem", color: "#7e92a4", fontFamily: MONO, fontSize: "0.7rem", whiteSpace: "pre-wrap", wordBreak: "break-word", background: "var(--bg)" }}>{ev.detail}</pre>
                  )}
                </div>
              );
            })}
      ...
      {toast && (
        <div data-testid="system-notice" role="status" style={{ position: "fixed", left: "50%", bottom: "1.6rem", transform: "translateX(-50%)", zIndex: 9997, display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.65rem 1.1rem", background: "#0a1018", border: "1px solid rgba(0,212,255,0.4)", borderRadius: 10, color: "#00d4ff", fontFamily: MONO, fontSize: "0.78rem", boxShadow: "0 12px 40px rgba(0,0,0,0.55)" }}>
