...
import { ApiError, addResourceLink, addShipNeeds, createOperation, createRecurrence, getPartnerships, setCqbTeams, setFighterSquads } from "../api/client";
...
export function WizardPage({ session }: { session: SessionResponse | null }) {
  ...
  return (
    <div data-testid="create-page" style={{ width: "100%" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "1.1rem" }}>
                ...
                {postMode === "edit" && (
                  <div data-testid="wiz-post-panels" style={{ display: "flex", flexDirection: "column", gap: "1.1rem" }}>
                    ...
                    <AnnouncePanel opId={createdId} guildId={guildId} csrf={csrf} onNotice={setNotice} />
                  </div>
                )}
              </div>
            )}
      ...
      {picker && (
        <div data-testid="template-picker-backdrop" className="fpw-sheet" style={{ position: "fixed", inset: 0, zIndex: 10000, background: "rgba(18, 20, 22,0.72)", display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "3vh 1rem" }} onClick={() => setPicker(false)}>
          <div ref={pickerRef} role="dialog" aria-modal="true" aria-labelledby="template-picker-title" data-testid="template-picker" className="fpw-sheet-panel" onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 1080, background: "var(--bg)", border: "1px solid var(--border-hi)", borderRadius: 14, padding: "1.4rem 1.5rem", boxShadow: "0 24px 70px rgba(0,0,0,0.6)" }}>
