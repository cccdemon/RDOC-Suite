...
import { resolveMode, withMode, type OperationMode } from "../operationMode";
...
export function OpDetailPage({ session }: { session: SessionResponse | null }) {
  ...
  // §6: viewing and managing are two modes of this route, not one long page with
  // the console bolted underneath. The mode is in the URL so reload, deep link
  // and Back all land in the same place; every legacy `?op=` link still opens
  // the management side (see operationMode.ts).
  const mode: OperationMode = resolveMode(searchParams, op?.canManage ?? false);
  function switchMode(next: OperationMode) {
    if (next === mode) return;
    setSearchParams(withMode(searchParams, next)); // push, so Back returns
  }
  ...
  return (
    <OperationShell
      op={op}
      mode={mode}
      onMode={op.canManage ? switchMode : undefined}
      dateLabel={fmtDate(op.scheduledAt, op.guild.timezone)}
    >
      {mode === "manage" && id ? (
        <OperatorConsole
          op={op}
